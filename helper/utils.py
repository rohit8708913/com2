...
async def CompVideo(bot, query, ffmpegcode, c_thumb, subtitle_file_path):
    ...
    try:
        ...
        await ms.edit("🗜 **Compressing...**")
        cmd = f"""ffmpeg -i "{dl}" {ffmpegcode} -vf "scale=ceil(iw/2)*2:ceil(ih/2)*2,subtitles={subtitle_file_path}" -preset ultrafast -threads 4 -bufsize 64M -movflags +faststart "{Output_Path}" -y"""
        print(f"Running FFmpeg command: {cmd}")  # Debugging
